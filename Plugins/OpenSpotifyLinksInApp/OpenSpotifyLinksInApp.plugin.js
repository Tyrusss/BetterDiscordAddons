...
module.exports = (_ => {
	const config = {
		"info": {
			...
			"author": "Tyrus",
			"version": "1.1.4",
			"description": "Opens Spotify Links in Steam instead of your Browser"
		},
		"changeLog": {
			"fixed": {
	...
	return (window.Lightcord && !Node.prototype.isPrototypeOf(window.Lightcord) || window.LightCord && !Node.prototype.isPrototypeOf(window.LightCord) || window.Astra && !Node.prototype.isPrototypeOf(window.Astra)) ? class {
		...
		const urls = {
			steam: ["https://open.spotify.com/", "https://spotify.com/"]
		};
		...
		return class OpenSteamLinksInApp extends Plugin {
			...
			openInSteam (url) {
				BDFDB.LibraryRequires.request(url, (error, response, body) => {
					if (BDFDB.LibraryRequires.electron.shell.openExternal("spotify://" + response.request.href));
